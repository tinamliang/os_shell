...
int padToNextMultipleOfThree(int num);
...
int interpreter(char* command_args[], int args_size){
	...
	else if (strcmp(command_args[0], "print_mem") == 0) {
		printShellMemory();
		return 0;
	}
...
int run(char* script){
	...
	int errCode = 0;
	FILE* first = fopen(script, "r");
	FILE *filename1 = fopen("backing_store/file1.txt", "w");
	int lines_in_file = 0;
	int first_two_frames = 0;
	int total_lines = 0;
	char buffer1[256];

	while (fgets(buffer1, 256, first)){
		fprintf(filename1, "%s", buffer1);
		lines_in_file++;
	}
	...
	int pad = padToNextMultipleOfThree(lines_in_file);
	if (pad != 0) {
		lines_in_file = pad;
	}
	...
	int* arr = malloc(sizeof(int) * (lines_in_file / 3));
	memset(arr, -1, sizeof(int) * (lines_in_file / 3));

	errCode = process_initialize("backing_store/file1.txt", arr, first_two_frames, total_lines, (lines_in_file / 3));
	if(errCode != 0){
...
int exec(char *fname1, char *fname2, char *fname3) {
	...
	if(fname1 != NULL){
		...
		// load in either one or two pages according to number of lines in file
		int pad = padToNextMultipleOfThree(lines_in_file);
		if (pad != 0) {
			lines_in_file = pad;
		}
		...
		// allocate page table array for each process
		int* arr = malloc(sizeof(int) * (lines_in_file / 3));
		memset(arr, -1, sizeof(int) * (lines_in_file / 3));

        error_code = process_initialize("backing_store/file1.txt", arr, first_two_frames, total_lines, (lines_in_file / 3));
		if(error_code != 0){
			return handle_error(error_code);
		}
    }
    if(fname2 != NULL){
		FILE* snd = fopen(fname2, "r");
		FILE* filename2 = fopen("backing_store/file2.txt", "w");

        int lines_in_file1 = 0;
		int first_two_frames1 = 0;
		int total_lines1 = 0;
		char buffer2[256];

		while (fgets(buffer2, 256, snd)){
			fprintf(filename2, "%s", buffer2);
			lines_in_file1++;
		}

		total_lines1 = lines_in_file1;

		fclose(snd);
		fflush(filename2);

		int pad = padToNextMultipleOfThree(lines_in_file1);
		if (pad != 0) {
			lines_in_file1 = pad;
		}
		...
		int* arr1 = malloc(sizeof(int) * (lines_in_file1 / 3));
		memset(arr1, -1, sizeof(int) * (lines_in_file1 / 3));

        error_code = process_initialize("backing_store/file2.txt", arr1, first_two_frames1, total_lines1, (lines_in_file1 / 3));
		if(error_code != 0){
			return handle_error(error_code);
		}
    }
    if(fname3 != NULL){
        FILE* third = fopen(fname3, "r");
		FILE *filename3 = fopen("backing_store/file3.txt", "w");
		int lines_in_file3 = 0;
		int first_two_frames3 = 0;
		int total_lines3 = 0;
		char buffer3[256];

		while (fgets(buffer3, 256, third)){
			fprintf(filename3, "%s", buffer3);
			lines_in_file3++;
		}

		total_lines3 = lines_in_file3;

		fclose(third);
		fflush(filename3);

		int pad = padToNextMultipleOfThree(lines_in_file3);
		if (pad != 0) {
			lines_in_file3 = pad;
		}
		...
		int* arr3 = malloc(sizeof(int) * (lines_in_file3 / 3));
		memset(arr3, -1, sizeof(int) * (lines_in_file3 / 3));

        error_code = process_initialize("backing_store/file3.txt", arr3, first_two_frames3, total_lines3, (lines_in_file3 / 3));
		if(error_code != 0){
			return handle_error(error_code);
		}
    } 
